public class Location implements Comparable {
	...
	public Location getAdjacentLocation(int direction) {
		...
		int modifiedDirection = (direction + HALF_RIGHT / 2) % FULL_CIRCLE;
		if (modifiedDirection < 0)
			modifiedDirection += FULL_CIRCLE;

		modifiedDirection = (modifiedDirection / HALF_RIGHT) * HALF_RIGHT;
		int dc = 0;
		int dr = 0;
		if (modifiedDirection == EAST)
			dc = 1;
		else if (modifiedDirection == SOUTHEAST) {
			dc = 1;
			dr = 1;
		} else if (modifiedDirection == SOUTH)
			dr = 1;
		else if (modifiedDirection == SOUTHWEST) {
			dc = -1;
			dr = 1;
		} else if (modifiedDirection == WEST)
			dc = -1;
		else if (modifiedDirection == NORTHWEST) {
			dc = -1;
			dr = -1;
		} else if (modifiedDirection == NORTH)
			dr = -1;
		else if (modifiedDirection == NORTHEAST) {
			dc = 1;
			dr = -1;
		}
	...
	public int getDirectionToward(Location target) {
		...
		return (Angle / HALF_RIGHT) * HALF_RIGHT;
	}
	...
	public boolean equals(Object other) {
		...
		Location adjacentLocation = (Location) other;
		return getRow() == adjacentLocation.getRow() && getCol() == adjacentLocation.getCol();
	}
	...
	public int compareTo(Object other) {
		Location adjacentLocation = (Location) other;
		if (getRow() < adjacentLocation.getRow())
			return -1;
		if (getRow() > adjacentLocation.getRow())
			return 1;
		if (getCol() < adjacentLocation.getCol())
			return -1;
		if (getCol() > adjacentLocation.getCol())
			return 1;
		return 0;
	...
	public void setNextRow(int r, int d) {
		if(r != 60) { //When you want to manipulate the existing location
			row = r;
		}
		if(d == 1) {
			row++;
	...
	public void setNextCol(int c,int d) {
		if(c != 60) { //When you want to manipulate the existing location
			col = c;
		}
		if(d == 2) {
			col++;
